// Ported from EJClaw (phj1081/EJClaw) — app-server-state.ts
  ...
  if (event.method === 'item/completed') {
    ...
    if (item.type === 'agentMessage') {
      ...
      if (item.phase === 'final_answer') {
        return { ...state, finalAnswer: text, latestAgentMessage: text };
      }
      return { ...state, latestAgentMessage: text };
    }

    if (item.type === 'contextCompaction') {
      return { ...state, compactionCompleted: true };
    }
