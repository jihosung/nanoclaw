...
If an Anthropic secret exists, tell the user OneCLI is already configured and working. Use Ask:
...
Ask: "These credentials are used by container agents for outbound API calls. Moving them to the vault means agents never see the raw keys, and you can apply rate limits and policies."
...
Ask: Do you want to use your **Claude subscription** (Pro/Max) or an **Anthropic API key**?
...
Once they have the token, ask the user with two options:
