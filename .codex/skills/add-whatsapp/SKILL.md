...
Ask the user to collect configuration. **Adapt auth options based on environment:**

If IS_HEADLESS=true AND not WSL → Ask: How do you want to authenticate WhatsApp?
- **Pairing code** (Recommended) - Enter a numeric code on your phone (no camera needed, requires phone number)
- **QR code in terminal** - Displays QR code in the terminal (can be too small on some displays)

Otherwise (macOS, desktop Linux, or WSL) → Ask: How do you want to authenticate WhatsApp?
...
Ask: What is your phone number? (Digits only — country code followed by your 10-digit number, no + prefix, spaces, or dashes. Example: 14155551234 where 1 is the US country code and 4155551234 is the phone number.)
...
Ask: Is this a shared phone number (personal WhatsApp) or a dedicated number (separate device)?
...
Ask: What should the assistant call itself?
- **Andy** - Default name
- **Claw** - Short and easy
- **Claude** - Match the AI name

Ask: Where do you want to chat with the assistant?
...
The output shows `JID|GroupName` pairs. Present candidates as ask the user (names only, not JIDs).
